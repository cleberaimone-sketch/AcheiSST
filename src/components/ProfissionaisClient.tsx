...
import { Search, MapPin, Filter, Award, UserCircle, CheckCircle2, Mail, Phone, Linkedin } from 'lucide-react'
...
export function ProfissionaisClient({ profissionais }: Props) {
  ...
  const filtered = profissionais.filter((p) => {
    ...
    if (search.trim()) {
      ...
      if (!p.nome.toLowerCase().includes(t) && !p.especialidade.toLowerCase().includes(t) && !(p.bio ?? '').toLowerCase().includes(t)) return false
    }
    return true
  })
    ...
    <section className={`${isV5 ? 'bg-slate-950' : 'bg-slate-50'} min-h-screen transition-colors`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-64 flex-shrink-0">
          <div className={`${isV5 ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} p-6 rounded-2xl border shadow-sm sticky top-24 transition-colors`}>
            ...
            <button
              onClick={() => { setUf('Todos'); setEspecialidade('Todos'); setSearch('') }}
...
function ProfissionalCard({ prof }: { prof: Profissional }) {
  return (
    <article className="bg-white border border-slate-200 rounded-3xl p-6 flex gap-5 hover:shadow-lg hover:shadow-slate-200 transition-all duration-300 group">
      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center flex-shrink-0">
        {prof.avatar_url
          ? <img src={prof.avatar_url} alt={prof.nome} className="w-full h-full rounded-2xl object-cover" />
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          {prof.verified && (
            <CheckCircle2 className="w-4 h-4 text-green-600 flex-shrink-0 mt-0.5" aria-label="Perfil verificado" />
          )}
        </div>
        ...
        <div className="flex items-center gap-1 text-slate-500 text-xs mb-3">
          ...
          {prof.registro && <span className="text-slate-400">• {prof.registro}</span>}
        </div>

        {prof.bio && (
          <p className="text-slate-600 text-sm leading-relaxed line-clamp-2 mb-4">{prof.bio}</p>
        )}
